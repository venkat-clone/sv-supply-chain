...
const TableComponent = ({
    ...
    const { data, isLoading, isError, error, isFetching } = useQuery({
        queryKey: [`table-${tableKey}`, page, combinedFilters, sortBy, sortDirection],
        queryFn: () =>
            getAllFn({ page, filters: combinedFilters, sortBy, sortOrder: sortDirection }),
    ...
    return (
        <div className="p-6 bg-white shadow rounded-lg">
            ...
            redirect ? "hover:bg-gray-50 cursor-pointer" : ""
                                ...
                                onClick={
                                    redirect ? () => router.push(`${redirect}/${row.id}`) : undefined
                                }
                            >
                                {headers.map((header) => (
                                    <td key={header.key} className="px-4 py-2 text-sm text-gray-800">
