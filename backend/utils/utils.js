    ...
    validationResult.error.errors.forEach(error => {
        error.path.forEach(path => {
            if (error.received === 'undefined' || error.received === undefined) {
                ...
                userFriendlyErrors[path] = `${path} should be ${error.expected}, provided ${error.received}`;
            }
...
const getPrismaErrorMessage = (error) => {
    console.log(`[ProductRepository] Error creating getPrismaErrorMessage`, error);
    // if (!error.code){
...
const prismaErrorHandler = (error, req, res, next) => {
    console.log(error);
    console.log(`Prisma Error Handler ${error instanceof Prisma.PrismaClientKnownRequestError}`);
    ...
    if(error.name === 'PrismaClientValidationError'){
        return res.status(400).json({success: false, message: `Invalid data format.`, meta: error});
    }
    let {message, statusCode} = getPrismaErrorMessage(error);
    message = message || "An unexpected error occurred";
    statusCode = statusCode || 500;
    return res.status(statusCode).json({success: false, message, meta: error.meta});
};
